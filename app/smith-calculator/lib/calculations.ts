 ...
 */
function calculateBasePotentialIncrease(
...
function calculatePotentialIncrease(input: SmithingInput): number {
	...
	// キャラステータスによる潜在値上昇量
	const basePotentialIncrease = calculateBasePotentialIncrease(
		input.equipmentType,
		characterStats,
	)

	// 丁寧な制作補正（基礎潜在値に対して、小数点以下切り捨て）
	const carefulCraftingBonus = intFloor(
		(basePotential ?? 0) * (skills.carefulCrafting / 100),
	)

	// 匠の製作技術補正（基礎潜在値に対して、小数点以下切り捨て）
	const masterCraftingBonus = intFloor(
		(basePotential ?? 0) * ((skills.masterCrafting * 2) / 100),
	)
